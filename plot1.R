...
## Calculate sum of PM2.5 for each year and create new data frame with the values

pmSum <- with(NEI, tapply(Emissions, year, sum, na.rm = TRUE))
names(pmSum)
...
plot(pmYear$Year, pmYear$Emissions, type = "l", xlab = "Year", ylab = "Total Emissions",
     main = "Total Emissions (PM2.5) per Year", xaxt = "n")
